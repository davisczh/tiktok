...
def create_filters_from_conditions(conditions):
    ...
    return filters
def interactive_recommendation(conditions=None):
    #example conditions
    conditions = {
        'category': 'Electronic',
    ...
    # querying with filters is not tested
    filters = create_filters_from_conditions(conditions)
    print(filters)
    print("Welcome to the interactive product recommendation system!")
    print("For each product, enter 'y' if you like it, 'n' if you don't, or 'f' if you've found your target product.")
    ...
    while True:
        ...
        if not recommendations:
            ...
            return
        ### TO BE REPLACED BY FRONTEND: provide recommendations to user and received feedback
        for product_id in recommendations:
            product_name = products_df.loc[products_df['asin'] == product_id, 'title'].values[0]
            print(f"\nRecommended: {product_name}")
            
            while True:
                user_input = input("Do you like this product? (y/n/f/q): ").lower()
                if user_input in ['y', 'n', 'f', 'q']:
                    break
                print("Invalid input. Please enter 'y', 'n', 'f', or 'q'.")

            if user_input == 'q':
                print("Thank you for using the recommendation system!")
                return
            elif user_input == 'f':
                print(f"Great! You've found your target product: {product_name}")
                return
            elif user_input == 'y':
        ...
        negative_embeddings = get_vectors_for_asins(client, "amazon_products", negative_ids)
        positive_embeddings = get_vectors_for_asins(client, "amazon_products", positive_ids)
